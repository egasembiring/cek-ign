...
import { pubgm } from "@/handlers/pubgm";

export default new Elysia({ name: "pubgm" })
  .model({
    "pubgm.query": t.Object({
      id: t.String(),
    }),
    "pubgm.response": t.Object({
      success: t.Boolean(),
      code: t.Numeric(),
      data: t.Object({
        game: t.String(),
  ...
  .get("/pubgm", ({ query: { id } }) => pubgm({ id }), {
    query: "pubgm.query",
    response: "pubgm.response",
